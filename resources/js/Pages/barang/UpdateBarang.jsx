...
const UpdateBarang = ({ user, barangs }) => {

    const baseUrl = 'http://127.0.0.1:8000';
    // Define state
    const [nama, setNama] = useState(barangs.nama || '');
    const [harga, setHarga] = useState(barangs.harga || '');
    const [deskripsi, setDeskripsi] = useState(barangs.deskripsi || '');
    const [gambarPreview, setGambarPreview] = useState(barangs.gambar ? `${baseUrl}/${barangs.gambar}` : null); // Initialize with existing image URL if available
    ...
    const updatePost = async (e) => {
        ...
        Inertia.post(`/barangs/${barangs.id}`, formData, {
